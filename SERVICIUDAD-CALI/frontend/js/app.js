...
async function consultarDeuda() {
    ...
    try {
        ...
        console.log('🔐 Realizando petición con autenticación a:', `${API_CONFIG.baseUrl}/api/deuda/cliente/${clienteId}`);
        ...
        console.log('💰 Total General recibido:', data.totalGeneral, 'Tipo:', typeof data.totalGeneral);
...
function renderEstadisticas(stats) {
    ...
    const estadisticas = [
        ...
        { 
            icon: '🚰', 
            label: 'Consumo Total Acueducto', 
            value: totalConsumoAcueducto > 0 ? `${totalConsumoAcueducto.toFixed(1)} m³` : '0.0 m³'
        },
        { 
            icon: '⚡', 
            label: 'Deuda Energía', 
        ...
        { 
            icon: '💡', 
            label: 'Consumo Total Energía', 
            value: totalConsumoEnergia > 0 ? `${totalConsumoEnergia.toFixed(1)} kWh` : '0.0 kWh'
        },
        { 
            icon: '📈', 
            label: 'Promedio Acueducto', 
            value: promedioAcueducto > 0 ? `${promedioAcueducto.toFixed(1)} m³` : '0.0 m³'
        }
    ];
        ...
        facturaCard.innerHTML = `
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 10px; margin-top: 10px;">
                <div>
                    ...
                    <div>${consumo.toFixed(1)} m³</div>
                </div>
                <div>
                    <div class="result-label">Valor</div>
                    <div><strong>${formatearMoneda(valorPagar)}</strong></div>
                </div>
                <div>
...
function renderConsumosEnergia(consumos) {
    ...
    consumos.forEach(consumo => {
        ...
        // Determinar si es válido basado en si tiene consumo > 0
        const consumoKwh = parseFloat(consumo.consumo) || parseFloat(consumo.consumoKwh) || 0;
        const valorPagar = parseFloat(consumo.valorPagar) || parseFloat(consumo.valor) || 0;
        const esValido = consumoKwh > 0;
        
        energiaCard.innerHTML = `
            <div class="factura-header">
                <strong>⚡ Periodo ${consumo.periodo}</strong>
                ${esValido ? '<span class="estado-badge estado-pagada">✓ Válido</span>' : '<span class="estado-badge estado-vencida">✗ Inválido</span>'}
            </div>
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 10px; margin-top: 10px;">
                <div>
                    <div class="result-label">Consumo</div>
                    <div>${consumoKwh.toFixed(1)} kWh</div>
                </div>
                <div>
                    <div class="result-label">Valor</div>
                    <div><strong>${formatearMoneda(valorPagar)}</strong></div>
...
function copyClientId(clienteId) {
    ...
    navigator.clipboard.writeText(clienteId).then(() => {
        ...
        // Focus en el botón de consultar
        document.getElementById('consultarBtn').focus();
    }).catch(err => {
        console.error('Error al copiar:', err);
        // Fallback: solo pegar en el input
        document.getElementById('clienteId').value = clienteId;
    });
}
...
window.addEventListener('load', () => {
    console.log('✅ Página cargada, iniciando verificación de Docker...');
    verificarConexionDocker();
});
